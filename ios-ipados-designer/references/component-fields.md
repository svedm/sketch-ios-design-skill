...
|---|---|---|---|---|
| **List row** | `Lists/{L}/Rows/Default` (`Large`, `Subtitle`, `Value`, …) | `Stack/Title`, `Stack/Trailing/Detail` text; **`Accessories/Image` slot** (`property:image` — drop a rendered SF-Symbol PNG straight in, no glyph needed); `Stack/Trailing/Chevron` (drill-in) keep visible; **hide `Accessories/Edit Action`** (the red delete accessory is ON by default). **Selected-row tint = the row's own root background override** `<instanceId>_color:fill-0` — do NOT draw a tinted rect *under* the instance (the row's internal bg covers it; the underlay renders fine alone but never composites through) | row style; accessory type | `List`/`Section` row; `Label`, `.badge`, accessory; selection |
...
| **Slider** | `Sliders/{L}/{Glyphs\|No Glyphs}/{Min\|Mid\|Max} - {1 - Enabled\|4 - Disabled}` | `Min symbol`/`Max symbol` glyphs; nested `Knob`,`Track` (value = Min/Mid/Max leaf); hide `Ticks`/`Tick` isVisible (dots ON by default) | value position × enabled | `Slider(value:in:)` |
| | **Slider gotcha:** natural width 402; **resizing the instance breaks it** — the Knob pins to the right edge, Fill/Track de-align. `detach({recursively:true})` doesn't help: the inner `Stack` group keeps flex layout and re-flows your frames. For a custom-width slider at an exact value, **draw it in kit metrics instead**: track `h6 r3 #78788029`, fill `h6 r3 #007AFF` (disabled `#A9CDF8`) to value·width, knob = 26pt white oval, border 0.5 `#00000014` outside, shadow 0/2/6 `#00000033`. | | | |
...
| | **Selected segment ≠ Item 1:** each `Item N` is a nested symbol — swap `symbolID` between the `… - Selected` and `… - Unselected` masters (harvest both ids from a placed instance: Item 1 defaults to Selected, Items 2–5 to Unselected). The swap **re-paths the nested `Label` override** (Selected and Unselected masters have different inner Label ids) — set labels *after* the swap using the other master's Label id. Resizes cleanly (unlike Slider). | | | |
